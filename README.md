...
```
/aja-job-cv 001-company-role "Senior Software Engineer"
```
Use the job ID shown at the end of Step 1. The assistant rewrites your summary and reorders your experience bullets to match this job's requirements.
...
```
If you already ran `/aja-match-job` for this job, pass the job ID as a third argument to link them:
```
/aja-track https://example.com/jobs/some-role "Senior Software Engineer" 001-company-role
```
...
```
/aja-prep 001-company-role tech
```
Replace `tech` with `hr` or `non-tech` depending on the stage. The assistant will:
...
```
/aja-post-interview 001-company-role tech
```
